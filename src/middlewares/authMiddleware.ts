...
export const requireAuth = (
  ...
  if (!authHeader?.startsWith('Bearer ')) {
    res
      .status(401)
      .json({ success: false, message: 'Token no proporcionado o inválido' });
    return;
  }
  ...
  try {
    ...
    res
      .status(401)
      .json({ success: false, message: 'Token expirado o inválido' });
  }
};
...
export const requirePermission = (permisoClave: string) => {
  return async (
    ...
    try {
      ...
      res
        .status(500)
        .json({ success: false, message: 'Error al verificar permisos' });
    }
  };
};
