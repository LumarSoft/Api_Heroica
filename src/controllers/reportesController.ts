...
export const getReportesBySucursal = async (req: Request, res: Response) => {
  try {
    ...
    movimientos.forEach((mov: any) => {
      ...
      if (mov.tipo === 'ingreso') {
        ...
        ingresosPorCategoria[catNombre].subcategorias[subCatNombre] =
          (ingresosPorCategoria[catNombre].subcategorias[subCatNombre] || 0) + monto
        ...
        egresosPorCategoria[catNombre].subcategorias[subCatNombre] =
          (egresosPorCategoria[catNombre].subcategorias[subCatNombre] || 0) + montoAbs
    ...
    const formatBreakdown = (
      breakdownRecord: Record<string, { total: number; subcategorias: Record<string, number> }>,
    ) => {
    ...
    const creditosTotales = creditosList.reduce((acc: number, mov: any) => acc + Math.abs(Number(mov.monto)), 0)
