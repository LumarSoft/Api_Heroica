...
export const createCategoria = async (req: Request, res: Response) => {
  try {
    ...
    const created: any = await query('SELECT * FROM categorias WHERE id = ?', [
      result.insertId,
    ]);
...
export const updateCategoria = async (req: Request, res: Response) => {
  try {
    ...
    const updated: any = await query('SELECT * FROM categorias WHERE id = ?', [
      id,
    ]);
...
export const deleteCategoria = async (req: Request, res: Response) => {
  try {
    ...
    await query(
      'UPDATE subcategorias SET deleted_at = NOW() WHERE categoria_id = ? AND deleted_at IS NULL',
      [id],
    );
...
export const deleteSubcategoria = async (req: Request, res: Response) => {
  try {
    ...
    await query('UPDATE subcategorias SET deleted_at = NOW() WHERE id = ?', [
      id,
    ]);
...
export const createBanco = async (req: Request, res: Response) => {
  try {
    ...
    const created: any = await query('SELECT * FROM bancos WHERE id = ?', [
      result.insertId,
    ]);
...
export const createMedioPago = async (req: Request, res: Response) => {
  try {
    ...
    const created: any = await query('SELECT * FROM medios_pago WHERE id = ?', [
      result.insertId,
    ]);
...
export const updateMedioPago = async (req: Request, res: Response) => {
  try {
    ...
    const updated: any = await query('SELECT * FROM medios_pago WHERE id = ?', [
      id,
    ]);
...
export const updateUsuarioRol = async (req: Request, res: Response) => {
  try {
    ...
    const rolExiste: any = await query('SELECT id FROM roles WHERE id = ?', [
      rol_id,
    ]);
...
export const toggleUsuarioActivo = async (req: Request, res: Response) => {
  try {
    ...
    await query('UPDATE usuarios SET activo = ? WHERE id = ?', [
      nuevoEstado,
      id,
    ]);
...
export const createUsuario = async (req: Request, res: Response) => {
  try {
    ...
    if (Array.isArray(sucursal_ids) && sucursal_ids.length > 0) {
      ...
      const flatParams = sucursal_ids.flatMap((sid: number) => [
        newUserId,
        sid,
      ]);
      await query(
...
export const updateRol = async (req: Request, res: Response) => {
  try {
    ...
    if (Array.isArray(permiso_ids) && permiso_ids.length > 0) {
      const placeholders = permiso_ids.map(() => '(?, ?)').join(', ');
      const flatParams = permiso_ids.flatMap((pid: number) => [
        Number(id),
        pid,
      ]);
      await query(
...
export const getPermisos = async (req: Request, res: Response) => {
  try {
    ...
    res
      .status(500)
      .json({ success: false, message: 'Error al obtener permisos' });
  }
};
...
export const updateUsuarioSucursales = async (req: Request, res: Response) => {
  try {
    ...
    res
      .status(500)
      .json({ success: false, message: 'Error al actualizar sucursales' });
  }
};
