...
export const uploadDocumento = async (req: Request, res: Response) => {
  try {
    ...
    const existingResult: any = await query('SELECT id FROM sucursales WHERE id = ?', [id])
...
export const deleteDocumento = async (req: Request, res: Response) => {
  try {
    ...
    const result: any = await query('SELECT * FROM documentos_sucursal WHERE id = ? AND sucursal_id = ?', [
      docId,
      sucursalId,
    ])
