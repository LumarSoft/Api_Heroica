...
export const uploadDocumento = async (req: Request, res: Response) => {
  try {
    ...
    const existingResult: any = await query('SELECT id FROM movimientos WHERE id = ?', [id])
...
export const deleteDocumento = async (req: Request, res: Response) => {
  try {
    ...
    const result: any = await query('SELECT * FROM documentos_movimiento WHERE id = ? AND movimiento_id = ?', [
      docId,
      movimientoId,
    ])
