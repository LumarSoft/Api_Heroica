...
export const updateCuentaBancaria = async (req: Request, res: Response) => {
  try {
    ...
    await query('UPDATE cuentas_bancarias_sucursal SET cbu = ?, alias = ?, tipo_cuenta = ?, banco = ? WHERE id = ?', [
      cbu,
      alias || null,
      tipo_cuenta || null,
      banco || null,
...
export const deleteCuentaBancaria = async (req: Request, res: Response) => {
  try {
    ...
    await query('UPDATE cuentas_bancarias_sucursal SET deleted_at = NOW() WHERE id = ?', [id])
