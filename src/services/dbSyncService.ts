...
const syncDatabase = async () => {
  ...
  console.log(
    '⏱️ Iniciando sincronización de base de datos oficial a base de pruebas...',
  );
  ...
  try {
    ...
    const [rows]: any = await connection.query(
      'SHOW TABLES FROM heroica_oficial;',
    );
    ...
    for (const table of tables) {
      ...
      await connection.query(
        `DROP TABLE IF EXISTS heroica_prueba.\`${table}\`;`,
      );
      await connection.query(
        `CREATE TABLE heroica_prueba.\`${table}\` LIKE heroica_oficial.\`${table}\`;`,
      );
      await connection.query(
        `INSERT INTO heroica_prueba.\`${table}\` SELECT * FROM heroica_oficial.\`${table}\`;`,
      );
    }
    ...
    console.log(
      `✅ Sincronización exitosa. Se copiaron ${tables.length} tablas en ${duration}ms.`,
    );
  } catch (error: any) {
    console.error(
      '❌ Error durante la sincronización de las bases de datos:',
      error.message,
    );
...
export const startDbSyncCron = () => {
  ...
  console.log(
    '📅 Tarea programada (CRON): Sincronización de BD diaria (06:00 y 18:00).',
  );
