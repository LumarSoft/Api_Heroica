...
const dir =
  'c:/Users/Bodin/OneDrive/Desktop/Heroica/Api_Heroica/src/controllers';
...
const pPC = fs.readFileSync(
  path.join(dir, 'pagosPendientesController.ts'),
  'utf8',
);

// remove imports from the second and third
const removeImports = (content) =>
  content
    .replace(/import \{ Request, Response \} [^\n]+\n/g, '')
    .replace(/import \{ query \} [^\n]+\n/g, '');

let cbcClean = removeImports(cBC);
cbcClean = cbcClean.replace(
  'export const moverAReal =',
  'export const moverARealBanco =',
);
...
// Now update routes
const routesDir =
  'c:/Users/Bodin/OneDrive/Desktop/Heroica/Api_Heroica/src/routes';
let cbR = fs.readFileSync(path.join(routesDir, 'cajaBancoRoutes.ts'), 'utf8');
cbR = cbR.replace(
  "'../controllers/cajaBancoController'",
...
let ppR = fs.readFileSync(
  path.join(routesDir, 'pagosPendientesRoutes.ts'),
  'utf8',
);
ppR = ppR.replace(
  "'../controllers/pagosPendientesController'",
  "'../controllers/movimientosController'",
);
